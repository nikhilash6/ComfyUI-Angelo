...
| **⬆ 2× Restore** | Two-stage, reference-anchored **2× creative upscale**: global restore at native res, then pixel-space 2× + a "high quality image" pass per ~1MP tile, feathered seams. Result goes through a review overlay (Accept / Try again / Cancel); **Accept = new session base, history resets** (dimension change, like Outpaint). Edit models + CLIP; Refine mode only |
