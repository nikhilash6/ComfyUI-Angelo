...
- **⬆ 2× Restore Upscale** — creative upscaling built from the same recipe: 2× pixel upscale, then restored tile by tile, each ~1MP tile anchored to its own content so tiles can't hallucinate and seams agree. Review before commit, like Outpaint.
...
| **⬆ 2× Restore** | Tiled, reference-anchored **2× creative upscale**: pixel-space 2× + the restore recipe per ~1MP tile, feathered seams. Result goes through a review overlay (Accept / Try again / Cancel); **Accept = new session base, history resets** (dimension change, like Outpaint). Edit models + CLIP; Refine mode only |
...
**⬆ 2× Restore Upscale** — the recipe's biggest trick. One press: the image is upscaled 2× in pixel space (lanczos), re-encoded, then **restored tile by tile** — overlapping ~1MP squares, each run at denoise 1.0 anchored to its own content as a reference, composited with feathered seams. The per-tile anchor is what makes this work where naive tiled upscalers fail: tiles can't hallucinate new content into skies and bokeh, and adjacent tiles agree because they're anchored to the same underlying image — the job tile-ControlNets do in other pipelines, free here. The result appears in a **review overlay** (Accept / Try again / Cancel); accepting commits it as a **new session base — history resets**, exactly like Outpaint, because the dimensions changed. A 1MP source becomes ~9 tiles, roughly 15–20 seconds on Klein.
