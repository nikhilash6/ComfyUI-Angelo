...
- **✨ Quick Photo Refine** — or skip the recipe entirely: one button, fixed magic settings (full canvas, a model-tuned restoration instruction, denoise 1.0, fully anchored). Load photo, press, done. Mash it with seed-randomize for variations; Undo any pass you don't like.
- **⬆ 2× Restore Upscale** — creative upscaling built from the same recipe, in two stages: restore globally at native res, then 2× pixel upscale refined tile by tile (each ~1MP tile anchored to its own content so tiles can't hallucinate and seams agree). Review before commit, like Outpaint.
...
```
 ...
 ─────────────────────────────────────────
  ...
  [✦ Quick Actions: ✨ Quick Photo Refine | ⬆ 2× Restore | ⬆ 2× Upscale | Faces]   ← one-press magic buttons
```
...
| **✨ Quick Photo Refine** | One-click photo restoration with a **fixed recipe**: whole image, a model-tuned restoration instruction (auto-detected per model), denoise 1.0, reference anchor 1.0 — identity stays, texture re-renders. Ignores every toolbar box except **Seed**. Auto-tiles on canvases over ~1.6MP. Edit models + CLIP; Refine mode only. See "Photo restoration" |
| **⬆ 2× Restore** | Two-stage, reference-anchored **2× creative upscale**: global restore at native res, then pixel-space 2× + a keep-colours/style restore pass per ~1400px tile, feathered seams. Result goes through a review overlay (Accept / Try again / Cancel); **Accept = new session base, history resets** (dimension change, like Outpaint). Edit models + CLIP; Refine mode only |
| **⬆ 2× Upscale** | The same tiled upscale **without the restoration pre-pass** — for images that are already clean (fresh generations, good photos) and just need size + crispness. Same review-before-commit flow |
...
**✨ Quick Photo Refine** (in the edit row, Refine mode only). A true magic button with its own **fixed recipe**: one press re-renders the **whole image** with a **model-tuned internal instruction** — *"restore the photo"* on FLUX-family models, *"lightly restore this old photo, remove dust and scratches, improve sharpness and contrast, preserve original feel"* on Qwen-Image-Edit (auto-detected, no setting) — at **denoise 1.0**, fully anchored to the current image as a reference (**strength 1.0**) — identity stays, texture re-renders. **No toolbar box affects it** — Denoise, Reference, Area Prompt, toggles: all ignored. Only the **Seed** applies, so leave Seed Ctrl on `randomize` and **mash the button to cycle restoration variations**, using Undo to step back through them (or `\` to compare against the original). Load photo → press → done. (Want different numbers? That's what the manual recipe below is for.)
...
**⬆ 2× Restore Upscale** — the recipe's biggest trick, in **two stages** that mirror a real photo workflow (repair → enlarge → sharpen). **Stage 1** restores the photo *globally* at native resolution — restoration is a global judgement (one consistent interpretation of the fading, the cast, the damage), so it happens with whole-image context using the model-tuned restore instruction. **Stage 2** upscales the clean image 2× in pixel space (lanczos), re-encodes, and refines it **tile by tile under a *"keep the colours, keep the style and restore the image"* prompt** — overlapping ~1400px tiles, each anchored to its own content as a reference, composited with feathered seams. The tiles' only job is rendering known-good content crisply at the new resolution; the per-tile anchor stops them hallucinating into skies and bokeh and keeps neighbours agreeing — the job tile-ControlNets do in other pipelines, free here. The result appears in a **review overlay** (Accept / Try again / Cancel); accepting commits it as a **new session base — history resets**, exactly like Outpaint, because the dimensions changed. A 1MP source ≈ one global pass + ~9 tiles, roughly 20–25 seconds on Klein. Image already clean (a fresh generation that just needs size)? **⬆ 2× Upscale** is the same pipeline minus stage 1.
...
**Automatic face polish.** Both upscale buttons finish with a face pass (when the optional [SAM 3 Detect](#detect--auto-segment-with-sam-3-optional) is installed — skipped silently otherwise): SAM finds every face in the result, and each one **between 250px and ~1MP** gets a centred, feathered circular refine with the tile prompt — at **ref 0.75 / denoise 0.7 on 2× Restore** (a restoration must not change who the person is, so faces anchor hard) and **ref 0.3 / denoise 0.55 on 2× Upscale** (clean sources; the face just needs crispness like everything else). Smaller faces don't have the pixels to benefit; bigger ones already have all the resolution they need. Context follows canvas size — at ≤1.6MP the stamp samples the full canvas so the model sees the whole image; above that it uses the Xtra-Fine crop path with a 64px context pad, so a 4MP canvas is never sampled whole per face. The polish lands *before* the review overlay, so what you Accept is the finished article, and Try-again re-rolls polish included. The console reports the count (`[Angelo face-polish] 2 face(s)…`). The **Faces** toggle on the Quick Actions bar (ON by default) removes the stage entirely — for non-portrait work, or when you'd rather polish faces by hand with Detect + Fix All.
