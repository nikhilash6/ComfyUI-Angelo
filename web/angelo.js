...
function attachPreviewCanvas(node) {
    ...
    quickFixBtn.title = "One-click photo restoration — a true magic button with its own fixed "
        + "recipe: whole image, restoration prompt (\"high quality photo\" + keep colouring/lighting), denoise 0.75, reference anchor 0.9. "
...
function triggerQuickPhotoRefine(node) {
    ...
    if (!ws) return;
    setWidget(ws, ((ws.value || 0) + 1) & 0x7FFFFFFF);
    _angeloToast("✨ Quick Photo Refine — restoring (denoise 0.75, reference 0.9)…");
