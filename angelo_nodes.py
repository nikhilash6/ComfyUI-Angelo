...
def _outpaint_prepare(
    *,
    ...
    Returns (latent_init, mask, snapped_amount_px).
    """
...
class AngeloRefine:
    ...
    def run(
        ...
        if new_outpaint:
            latent_init, op_mask, op_amt = _outpaint_prepare(
            ...
            # Conditioning: the Area Prompt (if armed) lets the user say
            # what lives in the new space ("a pier stretching into the
            # sea"); otherwise the main prompt carries the scene.
            if area_prompt and clip is not None:
                tokens_p = clip.tokenize(str(area_text_positive))
                ...
                op_negative = negative
            # Whole-original reference for edit models (Klein / Qwen):
            # continuing a scene in-style is their edit branch's strength.
            # REPLACE (append=False) per the Smart Inpaint lesson — and
            # non-edit models ignore the field, so it's harmless anywhere.
            op_positive = node_helpers.conditioning_set_values(
                op_positive, {"reference_latents": [current.clone()]}, append=False,
            )
